...
public:
    ...
    bool Intersects(const AARectangle& otherRect) const; // do sprawdzania czy prostokąty się przecinają
    bool ContainsPoint(const Vec2D& point) const; // sprawdzanie czy punkt znajduje się wewnątrz prostokąta

    static AARectangle Inset(const AARectangle& rect, Vec2D& insets); // statyczna metoda zracajaća prostokąt zmniejszony o podane wcięcia
